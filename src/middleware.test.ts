...
function makeContext(pathname: string, user: Record<string, unknown> | null = null) {
  ...
  return {
    ...
    cookies: { set: vi.fn() },
    redirect: vi.fn((url: string) => ({ redirectTo: url })),
...
describe("middleware", () => {
  ...
  describe("unauthenticated user", () => {
    ...
    it("redirects /dashboard to /auth/signin", async () => {
      ...
      await onRequest(ctx, next);
      expect((ctx as any).redirect).toHaveBeenCalledWith("/auth/signin");
      expect(next).not.toHaveBeenCalled();
    });

    it("redirects /campaigns to /auth/signin", async () => {
      const ctx = makeContext("/campaigns");
      await onRequest(ctx, next);
      expect((ctx as any).redirect).toHaveBeenCalledWith("/auth/signin");
      expect(next).not.toHaveBeenCalled();
    });
    ...
    it("redirects /campaigns/123/npcs to /auth/signin", async () => {
      const ctx = makeContext("/campaigns/123/npcs");
      await onRequest(ctx, next);
      expect((ctx as any).redirect).toHaveBeenCalledWith("/auth/signin");
    });
  });
  ...
  describe("authenticated user", () => {
    ...
    it("redirects / to /dashboard", async () => {
      ...
      await onRequest(ctx, next);
      expect((ctx as any).redirect).toHaveBeenCalledWith("/dashboard");
      expect(next).not.toHaveBeenCalled();
    });

    it("redirects /auth/signin to /dashboard", async () => {
      const ctx = makeContext("/auth/signin", user);
      await onRequest(ctx, next);
      expect((ctx as any).redirect).toHaveBeenCalledWith("/dashboard");
      expect(next).not.toHaveBeenCalled();
    });
    ...
    it("redirects /auth/signup to /dashboard", async () => {
      const ctx = makeContext("/auth/signup", user);
      await onRequest(ctx, next);
      expect((ctx as any).redirect).toHaveBeenCalledWith("/dashboard");
      expect(next).not.toHaveBeenCalled();
    });
