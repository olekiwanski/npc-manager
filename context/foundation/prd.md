...
### Secondary
- The NPC relationship graph renders as an interactive visual diagram and is navigable — the GM can see all NPCs in a campaign and their connections at a glance.
...
- FR-007: GM can view, edit, and delete an NPC. Priority: must-have
  > Socrates: No counter-argument; NPCs change over a campaign — edit and delete are both must-haves. What happens to an NPC's relationships when the NPC is deleted must be defined at implementation.
...
- FR-011: GM can view a visual graph of all NPCs and relationships in a campaign. Priority: nice-to-have (implement last; skip if time runs short)
  > Socrates: Counter-argument considered: "a graph rendering library adds a dependency and integration complexity for secondary value — defer to v2." Resolution: kept as a de-prioritized nice-to-have; the relationship data model is in place regardless; the visual rendering is added last and dropped if the timeline is tight.
...
No open questions — all sections fully populated from shape-notes. Quality check from shaping session: accepted.
